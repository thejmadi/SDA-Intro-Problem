...
if __name__ == '__main__':
    ...
    if is_multi:
        ...
        rng_class = None
    #print(np.sum(fc.SimulatePolicy(robots, sensors, optimize, is_multi, is_frozen = False, rng_class = rng_class)))
    ...
    for k in range(optimize.num_iter):
        ...
        for t in range(optimize.T-1):
            print("Timestep: ", t+1)
            if optimize.current_policy[:, t].any() != 1:
    ...
    if is_multi:
        ...
        for k in range(num_iter):
            ...
            optimize.Reset()
    '''
    plot.PlotHeatMapAnimation(optimize.all_policies, optimize.num_iter, "Updated Policy", "Time t", "Robot n", "GD_pol_logdet_1")
    plot.PlotHeatMapAnimation(optimize.all_grads, optimize.num_iter-1, "Gradients", "Time t", "Robot n", "GD_grad_logdet_1", is_grad = True)
